            ...
            <div class="cpp-video-player" data-cpp-video-player>
                ...
                if ($video->integration_type === 'presto' && !empty($video->presto_player_id)) {
                    ...
                    >
                        ...
                        <?php esc_html_e(__('Your browser does not support the video tag.', 'content-protect-pro'), 'content-protect-pro'); ?>
                    </video>
                    <?php
                }
                ?>
...
function cpp_shortcode_video_library($atts) {
    ...
    // Get query params
    $page = isset(sanitize_text_field($_GET['cpp_page'] ?? '')) ? absint(sanitize_text_field($_GET['cpp_page'] ?? '')) : 1;
    $search = isset(sanitize_text_field($_GET['cpp_search'] ?? '')) ? sanitize_text_field(sanitize_text_field($_GET['cpp_search'] ?? '')) : '';
    $access_filter = isset(sanitize_text_field($_GET['cpp_access'] ?? '')) ? absint(sanitize_text_field($_GET['cpp_access'] ?? '')) : 0;
    ...
    <div class="cpp-video-library-wrapper">
        ...
        <?php else: ?>
            <p class="cpp-no-results">
                <?php esc_html_e(__('No videos found.', 'content-protect-pro'), 'content-protect-pro'); ?>
            </p>
        <?php endif; ?>
    </div>
    <?php
