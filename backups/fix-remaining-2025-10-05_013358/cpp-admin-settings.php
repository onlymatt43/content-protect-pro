...
if (isset(sanitize_text_field($_POST['submit'] ?? ''))) {
    check_admin_referer('cpp_settings_nonce');
    
    // Save general settings
    if (isset(sanitize_text_field($_POST['cpp_general_settings'] ?? ''))) {
    ...
    if (isset(sanitize_text_field($_POST['cpp_security_settings'] ?? ''))) {
        // Sanitized security settings
        $in = sanitize_text_field($_POST['cpp_security_settings'] ?? '');
        $san = array();
    ...
    if (isset(sanitize_text_field($_POST['cpp_integration_settings'] ?? ''))) {
        // Sanitized integration settings
        $in = sanitize_text_field($_POST['cpp_integration_settings'] ?? '');
        $san = array();
...
// Get active tab
$active_tab = isset(sanitize_text_field($_GET['tab'] ?? '')) ? sanitize_text_field(sanitize_text_field($_GET['tab'] ?? '')) : 'general';
?>
