...
function cpp_ai_chat_handler($request) {
    ...
    if (empty($message)) {
        return new WP_REST_Response([
            'success' => false,
            'message' => __('Message is required.', 'content-protect-pro'),
        ], 400);
    }
    ...
    if (!get_option('cpp_ai_assistant_enabled')) {
        return new WP_REST_Response([
            'success' => false,
            'message' => __('AI Assistant is disabled.', 'content-protect-pro'),
        ], 403);
    }
...
function cpp_ai_ask_handler($request) {
    ...
    if (empty($question)) {
        return new WP_REST_Response([
            'success' => false,
            'message' => __('Question is required.', 'content-protect-pro'),
        ], 400);
    }
...
function cpp_ai_suggest_handler($request) {
    ...
    if (empty($context)) {
        return new WP_REST_Response([
            'success' => false,
            'message' => __('Context is required.', 'content-protect-pro'),
        ], 400);
    }
...
function cpp_ai_test_connection_handler($request) {
    ...
    if (empty($api_key_encrypted)) {
        return new WP_REST_Response([
            'success' => false,
            'message' => __('API key not configured.', 'content-protect-pro'),
        ], 400);
    }
    ...
    if (empty($api_key)) {
        return new WP_REST_Response([
            'success' => false,
            'message' => __('Failed to decrypt API key.', 'content-protect-pro'),
        ], 500);
    }
